...
All dimensions are mm. Defaults follow turtle_body's lib/params.scad (the
sibling ../turtle_body repository's shared dimension contract) -- see
generator/SYNC_PLAN.md item S-1.
Run with --defaults to generate without prompting, e.g.:
  python3 back_fin_generator.py --defaults --output turtle_rear_fin.scad
...
shaft_hole_diameter and shaft_hole_from_front are the M6 bolt that ties each
green bottle-holder shaft to the Ecojoiner Little John it replaces a Presser
on (see turtle_body CLAUDE.md TB-07). shaft_hole_diameter must equal the
Ecojoiner's own clearance hole (p_m6_clearance_d(), 6.4mm) and
shaft_hole_from_front is not a free tuning knob -- it is DERIVED from the
same TB-07 formula turtle_body uses (p_rear_shaft_hole_from_front()), so
that the shaft's hole lands exactly where the John's hole is. Leave
--shaft-hole-from-front unset to get the derived value; only pass it to
deliberately test a non-standard offset.
"""
...
TUNING = {
    ...
    "shaft_width": 59.0,
    "shaft_hole_diameter": 6.4,  # turtle_body lib/params.scad p_m6_clearance_d()
    "shaft_hole_from_front": None,  # None = derive via TB-07; see SCREW_SIDE_OFFSET below
    "fin_rear_tab_width": 15.0,
}

# Mirrors turtle_body lib/params.scad p_screw_side_offset() -- the mounting-
# hole inset from a John's outer end, shared by the Ecojoiner, ballast and
# rear-fin interfaces. Not a TUNING/CLI knob: it is the fixed geometry of the
# Ecojoiner frame this fin bolts to, not a rear-fin-specific choice.
SCREW_SIDE_OFFSET = 25.0


def default_shaft_hole_from_front(p: dict[str, float]) -> float:
    """TB-07: turtle_body's p_rear_shaft_hole_from_front(), simplified.

    p_rear_shaft_hole_from_front() = p_rear_shaft_len() - screw_side_offset
        - bottle_h + cap_h + port_height, and p_rear_shaft_len() = bottle_h
        + (2/3)*(fin_board_w - 2*t) - cap_h, so bottle_h and cap_h cancel and
        (with port_height == bottle_diameter) this reduces to:
    """
    t = p["wood_thickness"]
    return (2 / 3) * (p["fin_board_width"] - 2 * t) - SCREW_SIDE_OFFSET + p["bottle_diameter"]
...
def dimensions(p: dict[str, float]) -> dict[str, float]:
    ...
    hole_from_front = p.get("shaft_hole_from_front")
    if hole_from_front is None:
        hole_from_front = default_shaft_hole_from_front(p)
    return dict(fin_width=width, shaft_rear_x=rear, shaft_length=length,
                ...
                shaft_hole_from_front=hole_from_front)
...
def build_scad(overrides: dict[str, float] | None = None) -> str:
    ...
    if overrides:
        ...
        p.update(overrides)
    # shaft_hole_from_front defaults to None (derive via TB-07); resolve it
    # to a concrete value here, before validate()/the SCAD header both need
    # a real number.
    p["shaft_hole_from_front"] = dimensions(p)["shaft_hole_from_front"]
    validate(p)
...
def main() -> None:
    ...
    print(f"Shaft M6 hole from front (TB-07): {d['shaft_hole_from_front']:g} mm")
